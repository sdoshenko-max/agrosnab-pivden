      ...
      <div className="card !p-0 overflow-hidden">
        {highlightedProducts.map((p, i) => (
          <Link key={p.slug} href={`${base}/produkt/${p.slug}`}
            ...
            {p.saveFromOriginal && (
              <span className="text-accent font-bold text-sm shrink-0 hidden md:inline">−{p.saveFromOriginal}%</span>
            )}
            <div className="text-right shrink-0">
              <p className="font-bold text-brand">${p.priceCash}<span className="text-xs font-normal text-muted">/{p.unit}</span></p>
              <p className="text-[10px] text-muted">{t.productCard.priceCash}</p>
            </div>
