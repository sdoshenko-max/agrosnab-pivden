...
export function ProductPage({ product, lang }: { product: Product; lang: Lang }) {
  ...
  // Related: ті самі культура+етап, інші препарати
  ...
  const labels = lang === "uk"
    ? {
        back: "До каталогу",
        for: "Для культур",
        regulation: "Регламент застосування",
        related: "Часто беруть разом",
        crop: "Культура",
        target: "Призначення",
        rate: "Норма",
    ...
    : {
        back: "К каталогу",
        for: "Для культур",
        regulation: "Регламент применения",
    ...
    <>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              ...
              {product.saveFromOriginal && (
                <span className="text-accent font-bold text-sm">−{product.saveFromOriginal}% від оригіналу</span>
              )}
            </div>
            ...
            <div className="bg-brand/5 rounded-lg p-4 mb-4">
              ...
              {product.analog && (
                <p className="text-sm text-muted mt-2">
                  {t.productCard.analog}: <span className="font-semibold text-ink">{product.analog}</span>
                </p>
              )}
            </div>
            ...
            <div className="card !p-4 mb-5 bg-bg">
              <div className="flex justify-between items-baseline mb-2">
                ...
                <span className="text-2xl font-extrabold text-brand">
                  {cur}{product.priceVat}<span className="text-sm font-normal">/{unitLabel}</span>
                </span>
              </div>
              <div className="flex justify-between items-baseline">
                <span className="text-sm text-muted">{t.productCard.priceCash}</span>
                <span className="text-xl font-bold text-ink">
                  {cur}{product.priceCash}<span className="text-sm font-normal">/{unitLabel}</span>
                </span>
              </div>
            </div>
      ...
      {desc && (
        <section className="container-w py-8">
          <h2 className="text-xl font-bold mb-3">{lang === "uk" ? "Про препарат" : "О препарате"}</h2>
