        ...
        <section className="container-w py-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cultures.map(c => {
              ...
              return (
                <Link key={c.slug} href={`/kultury/${c.slug}`} className="card hover:border-brand hover:shadow-lg transition-all duration-200 group overflow-hidden !p-0">
                  {c.image ? (
                    <div className="aspect-[16/10] overflow-hidden bg-bg"><img src={c.image} alt={c.nameUk} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" /></div>
                  ...
                  <div className="p-5 group-hover:bg-brand/5 transition-colors">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h2 className="text-lg font-bold group-hover:text-brand transition-colors">{c.nameUk}</h2>
                    </div>
                    <p className="text-sm text-muted mb-3 line-clamp-2">{c.shortUk}</p>
                    <span className="text-sm font-semibold text-brand inline-flex items-center gap-1">{count} препаратів<ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" /></span>
                  </div>
                </Link>
              );
            })}
